...
function pcaK(vs, k) {                        // 上位k主成分へ射影（べき乗法・決定論的）
  ...
  return X.map(x => comps.map(cp => dot(x, cp)));
}
const pca2 = vs => pcaK(vs, 2);
...
function renderModels() {
  ...
  $('modelList').querySelectorAll('.model-card').forEach(card => {
    ...
    card.querySelector('.mdl-map').onclick = () => openModelMap(m);
  });
}

/* ---------- モデル学習データの2D/3Dマップ（シリーズ色分け・正常/異常・誤判定） ---------- */
let MAPVIEW = null;
function openModelMap(model) {
  const ms = labeledFor(model.space, model.sid);
  if (ms.length < 2) { alert('学習データが2件未満のため表示できません。'); return; }
  const pts = pcaK(ms.map(m => m.emb[model.space]), 3);
  const info = ms.map(m => { const j = judgeWithModel(model, m.emb[model.space]); const lab = labelOf(m); return { lab, pred: j.pred, correct: j.pred === lab }; });
  MAPVIEW = { model, ms, pts, info, dim: 2, rot: { ax: 0.5, ay: 0.6 }, series: new Set(ms.map(m => m.sid)), showNormal: true, showAbn: true, drag: null };
  const nErr = info.filter(o => !o.correct).length;
  $('mapDlgTitle').textContent = `🗺 ${model.name}`;
  $('mapDlgInfo').textContent = `${spaceName(model.space)} ／ 学習${ms.length}件（正常${info.filter(o => o.lab === 'normal').length}/異常${info.filter(o => o.lab === 'abnormal').length}） ／ 学習データでの誤判定 ${nErr}件`;
  $('mapDim2').classList.add('on'); $('mapDim3').classList.remove('on');
  buildMapFilters();
  $('mapDlg').showModal();
  drawModelMap();
}
function buildMapFilters() {
  const sids = [...new Set(MAPVIEW.ms.map(m => m.sid))];
  const el = $('mapDlgFilters');
  el.innerHTML = sids.map(sid => `<label class="check mapf"><input type="checkbox" data-sid="${sid}" checked><i class="dot" style="background:${seriesColor(sid)}"></i>${seriesName(sid)}</label>`).join('')
    + '<label class="check mapf"><input type="checkbox" id="mfNormal" checked>○正常</label>'
    + '<label class="check mapf"><input type="checkbox" id="mfAbn" checked>◆異常</label>';
  el.querySelectorAll('input[data-sid]').forEach(cb => cb.onchange = () => {
    const sid = +cb.dataset.sid; cb.checked ? MAPVIEW.series.add(sid) : MAPVIEW.series.delete(sid); drawModelMap();
  });
  $('mfNormal').onchange = e => { MAPVIEW.showNormal = e.target.checked; drawModelMap(); };
  $('mfAbn').onchange = e => { MAPVIEW.showAbn = e.target.checked; drawModelMap(); };
}
function drawModelMap() {
  const S = MAPVIEW; if (!S) return;
  const cv = $('mapDlgCv'), c = cv.getContext('2d');
  cv.width = cv.clientWidth || 600; cv.height = Math.max(320, Math.round(cv.width * 0.6));
  c.fillStyle = '#0d1620'; c.fillRect(0, 0, cv.width, cv.height);
  const k = S.pts[0].length, mins = [], maxs = [];
  for (let j = 0; j < k; j++) { let lo = 1e9, hi = -1e9; S.pts.forEach(p => { lo = Math.min(lo, p[j]); hi = Math.max(hi, p[j]); }); mins[j] = lo; maxs[j] = hi; }
  const norm = p => p.map((v, j) => ((v - mins[j]) / ((maxs[j] - mins[j]) || 1)) * 2 - 1);
  const cx = cv.width / 2, cy = cv.height / 2, R = Math.min(cx, cy) - 26;
  const list = [];
  S.pts.forEach((p, i) => {
    const m = S.ms[i], o = S.info[i];
    if (!S.series.has(m.sid)) return;
    if (o.lab === 'normal' && !S.showNormal) return;
    if (o.lab === 'abnormal' && !S.showAbn) return;
    const nr = norm(p);
    let X, Y, depth = 0;
    if (S.dim === 2) { X = nr[0]; Y = nr[1]; }
    else {
      const { ax, ay } = S.rot; const x = nr[0], y = nr[1], z = nr[2] || 0;
    ...
    list.push({ m, o, px: cx + X * R, py: cy - Y * R, depth });
  });
  list.sort((a, b) => a.depth - b.depth);        // 奥から描く
  list.forEach(d => {
    const boost = S.dim === 3 ? 0.7 + 0.3 * (d.depth + 1.8) / 3.6 : 1;
    drawModelMarker(c, d.px, d.py, d.m, d.o, boost);
  });
  if (!list.length) { c.fillStyle = '#8aa'; c.font = '13px sans-serif'; c.fillText('表示対象がありません（フィルタを確認）', 14, 26); }
}
function drawModelMarker(c, px, py, m, o, boost) {
  const col = seriesColor(m.sid), r = 6 * boost;
  c.lineWidth = 2.4;
  c.strokeStyle = o.correct ? 'rgba(105,240,150,.95)' : '#ff5252';   // 緑=正解 / 赤=誤判定
  c.fillStyle = col;
  if (o.lab === 'abnormal') {
    c.beginPath();
    c.moveTo(px, py - r - 1); c.lineTo(px + r + 1, py); c.lineTo(px, py + r + 1); c.lineTo(px - r - 1, py); c.closePath();
    c.fill(); c.stroke();
  } else {
    c.beginPath(); c.arc(px, py, r, 0, Math.PI * 2); c.fill(); c.stroke();
  }
}
window.openModelMap = openModelMap;
...
$('mapDlgClose').onclick = () => $('mapDlg').close();
$('mapDim2').onclick = () => { if (!MAPVIEW) return; MAPVIEW.dim = 2; $('mapDim2').classList.add('on'); $('mapDim3').classList.remove('on'); drawModelMap(); };
$('mapDim3').onclick = () => { if (!MAPVIEW) return; MAPVIEW.dim = 3; $('mapDim3').classList.add('on'); $('mapDim2').classList.remove('on'); drawModelMap(); };
$('mapDlgCv').addEventListener('pointerdown', e => { if (!MAPVIEW || MAPVIEW.dim !== 3) return; MAPVIEW.drag = { x: e.clientX, y: e.clientY, ax: MAPVIEW.rot.ax, ay: MAPVIEW.rot.ay }; });
window.addEventListener('pointermove', e => { if (!MAPVIEW || !MAPVIEW.drag) return; MAPVIEW.rot.ax = MAPVIEW.drag.ax + (e.clientY - MAPVIEW.drag.y) * 0.01; MAPVIEW.rot.ay = MAPVIEW.drag.ay + (e.clientX - MAPVIEW.drag.x) * 0.01; drawModelMap(); });
window.addEventListener('pointerup', () => { if (MAPVIEW) MAPVIEW.drag = null; });
